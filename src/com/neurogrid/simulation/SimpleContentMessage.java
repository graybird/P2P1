...
public class SimpleContentMessage extends ContentMessage
{
  private static final String cvsInfo = "$Id: SimpleContentMessage.java,v 1.2 2003/06/25 11:50:45 samjoseph Exp $";
  ...
  private static Category o_cat = Category.getInstance(SimpleContentMessage.class.getName());  
  ...
  public static void init(String p_conf)
  {
    ...
    o_cat.info("SimpleContentMessage logging Initialized");
  }
  ...
  /**
   * SimpleContentMessage Constructor - used when initially sending the message
   *
   ...
   */
  public SimpleContentMessage(int p_TTL, Keyword[] p_keywords, Document p_document, Node p_start)
    throws Exception
  {
    if(p_TTL < 1 ) throw new Exception("SimpleContentMessage Constructor: TTL must be 1 or more");
    if(p_keywords == null || p_keywords.length < 1 ) throw new Exception("SimpleContentMessage Constructor: must have at least one keyword");
    if(p_document == null) throw new Exception("SimpleContentMessage Constructor: Document is null");
    if(p_start == null) throw new Exception("SimpleContentMessage Constructor: Node is null");
    o_message_ID = getNewMessageID();
    o_TTL = p_TTL;
    o_keywords = p_keywords;
    o_document = p_document;
    o_start = p_start;
  ...
  /**
   * SimpleContentMessage Constructor - used when forwarding the message
   *
   ...
   * @param p_message
   */
  public SimpleContentMessage(ContentMessage p_message)
    throws Exception
  {
    if(p_message == null) throw new Exception("SimpleContentMessage Constructor: Message is null");
  ...
  public int getPossibleMatches(Network p_network)
    throws Exception
  {
    return (o_document.getLocation()).size();
  }
