...
public class FuzzyMessage extends com.neurogrid.simulation.root.Message
{
  private static final String cvsInfo = "$Id: FuzzyMessage.java,v 1.3 2003/06/25 11:50:46 samjoseph Exp $";
  public static String getCvsInfo()
  {
    return cvsInfo;
  }
  
  private static Category o_cat = Category.getInstance(FuzzyMessage.class.getName());  
  ...
  public static void init(String p_conf)
  {
    ...
    o_cat.info("FuzzyMessage logging Initialized");
  }
  ...
  /**
   * FuzzyMessage Constructor - used when initially sending the message
   *
   ...
   */
  public FuzzyMessage(int p_TTL, Keyword[] p_keywords, Node p_start)
    throws Exception
  {
    if(p_TTL < 1 ) throw new Exception("FuzzyMessage Constructor: TTL must be 1 or more");
    if(p_keywords == null || p_keywords.length < 1 ) throw new Exception("FuzzyMessage Constructor: must have at least one keyword");
    if(p_start == null) throw new Exception("FuzzyMessage Constructor: Node is null");
  ...
  /**
   * FuzzyMessage Constructor - used when forwarding the message
   *
   ...
   * @param p_message
   */
  public FuzzyMessage(Message p_message)
    throws Exception
  {
    if(p_message == null) throw new Exception("FuzzyMessage Constructor: Message is null");
